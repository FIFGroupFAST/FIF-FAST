...
public class AssetTransferPopupComposer extends SelectorComposer<Window> {
	private static Logger log = LoggerFactory.getLogger(AssetTransferPopupComposer.class);
	@Wire
	...
	private String orderBy = "A.ASSET_NUMBER ASC";
	...
	public void searchAsset(){
		pgListOfValue.setTotalSize(assetTransferService.countAssetSODtlByCriteria(searchCriteria()));
	...
	private void generateDataAndPaging() {
		List listData = assetTransferService.getAssetSODtlByCriteria(searchCriteria(),pgListOfValue.getPageSize(), pgListOfValue.getActivePage() * pgListOfValue.getPageSize(),action);
		for(int i=0;i<listData.size();i++){
			AssetTransferDetailDTO assetTransferDetailDTO = (AssetTransferDetailDTO)listData.get(i);
			AssetTransferDetailDTO assetTransferDetailDTOTemp = assetTransferService.getSOResultByAssetId(assetTransferDetailDTO.getAssetId());
	...
	class DelegateSearchLocation implements SerializableSearchDelegateDoubleCriteria<SSOALocation> {
		...
		public List<SSOALocation> findBySearchCriteria(String searchCriteriaCode,String searchCriteriaName, int limit, int offset) {
			...
			listLocation = assetTransferService.getLocationByExample(example, limit, offset);
			return listLocation;
		}
		...
		public int countBySearchCriteria(String searchCriteriaCode,String searchCriteriaName) {
			...
			example.setCompanyId(securityServiceImpl.getSecurityProfile().getWorkspaceCompanyId());
			example.createCriteria().andLocationNameLike(searchCriteriaName).andLocationCodeLike(searchCriteriaCode);
			return assetTransferService.countLocationByExample(example);
		}
