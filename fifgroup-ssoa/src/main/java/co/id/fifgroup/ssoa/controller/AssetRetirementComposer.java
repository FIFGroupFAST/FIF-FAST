...
public class AssetRetirementComposer extends SelectorComposer<Window> {
    ...
    private static Logger log = LoggerFactory.getLogger(AssetRetirementComposer.class);
	...
	public void doAfterCompose(Window comp) throws Exception {
		...
		/*lstAssetsAdd.addEventListener(Events.ON_CLOSE, new SerializableEventListener<Event>() {
			private static final long serialVersionUID = -91355005227901153L;

			@Override
			public void onEvent(Event event) throws Exception {
				//System.out.println("msk onEvent"+ event.getData());
				if(event.getData()!=null){
					lstAssetsAdd.setModel((ListModel<Retirement>)event.getData());
				}
			}
		});*/
	}
	...
	private void init(){
		...
		Branch branch1 = (Branch) retirementService.getBranchById(securityServiceImpl.getSecurityProfile().getBranchId(),securityServiceImpl.getSecurityProfile().getWorkspaceCompanyId());
		bnbBranchId= branch1.getBranchId();
		System.out.println("bnbBranchId" + bnbBranchId);
		Branch branch = (Branch) retirementService.getBranchById(securityServiceImpl.getSecurityProfile().getBranchId(),securityServiceImpl.getSecurityProfile().getWorkspaceCompanyId());
	...
	class DelegateSearch implements SerializableSearchDelegateDoubleCriteria<Branch> {
		...
		public List<Branch> findBySearchCriteria(String searchCriteriaCode,String searchCriteriaName, int limit, int offset) {
			...
			listBranch = retirementService.getBranchByExample(example, limit, offset);
		...
		public int countBySearchCriteria(String searchCriteriaCode,String searchCriteriaName) {
			...
			example.setCompanyId(securityServiceImpl.getSecurityProfile().getWorkspaceCompanyId());
			example.createCriteria().andBranchNameLike(searchCriteriaName).andBranchCodeLike(searchCriteriaCode);
			return retirementService.countBranchByExample(example);
		}
	...
	public void addNew() {
		...
		Executions.createComponents("~./hcms/ssoa/asset_retirement_add.zul", getSelf().getParent(), param);
		getSelf().detach();
	}
    ...
    private void search() {
    	...
    	if(!validate()) {
			try {
				pgListOfValue.setTotalSize(retirementService.countByExample(searchCriteria()));
	...
	private void generateDataAndPaging() {
		...
		listRetirementDto = retirementService.getRetirementDtoByExample(searchCriteria(),pgListOfValue.getPageSize(), pgListOfValue.getActivePage() * pgListOfValue.getPageSize());
    ...
    private void generateDataAndPagingTop() {
		...
		listRetirementDto = retirementService.getRetirementDtoByExample(searchCriteria(),pgListOfValueTop.getPageSize(), pgListOfValueTop.getActivePage() * pgListOfValueTop.getPageSize());
	...
	private void generateDataForDownload(){
		listRetirementDtoForDownload = retirementService.getRetirementDtoByExample(searchCriteria());
	...
	public void onPrintBast(ForwardEvent event) throws Exception{
		RetirementDTO retirementDTO = (RetirementDTO) event.getData();
		retirementService.printBAST(retirementDTO);
	...
	public void onCreateRV(ForwardEvent event) {
		...
		getSelf().detach();
		/*try {
			retirementService.createRV(retirementDTO);
