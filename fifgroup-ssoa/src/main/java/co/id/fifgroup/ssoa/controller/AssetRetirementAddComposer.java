...
public class AssetRetirementAddComposer extends SelectorComposer<Window> {
	...
	private void setTmpFilePath(){
		List<GlobalSettingDTO> dataImgPath = retirementService.getGlobalsettingByCode(SSOAConstants.imagePathTmp);
		if(dataImgPath!=null){
			tmpFile = dataImgPath.get(0).getGlobalSetting().getValue().toString();
		}
		List<GlobalSettingDTO> dataMaxSizeUpload = retirementService.getGlobalsettingByCode(SSOAConstants.maxSizeUploadFileRetirement);
		if(dataMaxSizeUpload!=null){
			maxSize = dataMaxSizeUpload.get(0).getGlobalSetting().getValue().toString();
		}
		List<GlobalSettingDTO> dataMaxOriginalCostForLVA = retirementService.getGlobalsettingByCode(SSOAConstants.maxOriginalCostForLVA);
	...
	private void init(){
		...
		if(retirementDTO == null){
			...
			for(int i=0;i<retirementDTO.getRetirementDetail().size();i++){
				...
				if(listAsset.size()>0){
					...
					/*if(i==0){
						if(listAsset.get(0).getOriginalCost()!=null && listAsset.get(0).getOriginalCost().longValue() < new Long(maxOriginalCostForLVA)){
							assetType = SSOAConstants.assetTypeLVA;
						}
						if(listAsset.get(0).getOriginalCost()!=null && listAsset.get(0).getOriginalCost().longValue() >= new Long(maxOriginalCostForLVA)){
							assetType = SSOAConstants.assetTypeAST;
						}
						if(listAsset.get(0).getOriginalCost()==null){
							assetType = SSOAConstants.assetTypeLVA;
						}
					}*/
		...
		if(action.equals("detail")){
			...
			listDetailBast = retirementService.getDetailBastByPrimaryKey(retirementBast.getId());
			listModelBast.clear();
			...
			Messagebox.show(Labels.getLabel("sam.submitMessage"), "Information", Messagebox.YES | Messagebox.NO,
				...
				public void onEvent(Event event) throws Exception {
					...
					if(securityServiceImpl.getSecurityProfile().getBranchId().intValue() == -1 && retirementDTO.getBranchId().intValue()!= -1){
						...
						try {
						    retirementService.save(retirementDTO, personUUID,listDetail,lstRetirementQuotation.getValue());
				...
				Messagebox.QUESTION, new SerializableEventListener<Event>() {
					...
					public void onEvent(Event event) throws Exception {
						...
						if (status == 16) {
							Map<String, Object> param = new HashMap<String, Object>();
							param.put(GlobalVariable.FUNCTION_PERMISSION_KEY, functionPermission);
							Executions.createComponents("~./hcms/ssoa/asset_retirement.zul", getSelf().getParent(), param);
							getSelf().detach();
						}
					}
				});
	...
	public void onViewImage(ForwardEvent event){
		...
		retirementDTO = (RetirementDTO) arg.get("retirementDTO");
		  ...
		  if(action.equals("detail")){
			...
			RetirementDetailDTO retirementDetail = retirementService.getDetailIdForImages(id, assetId);
			...
			Window win = (Window) Executions.createComponents("~./hcms/ssoa/asset_retirement_detail_image.zul", getSelf().getParent(), param);
			win.doModal();
		  }
		...
		if(lstdetail == null || lstdetail.getModel() == null || lstdetail.getModel().getSize() == 0){
			...
			for(int i=0;i<lstdetail.getModel().getSize();i++){
				...
				if(retirementDetailDTO.getRetirementImg() == null || retirementDetailDTO.getRetirementImg().size()==0){
					ErrorMessageUtil.setErrorMessage(errMsgDtl, "Image for Asset Number "+retirementDetailDTO.getAssetNumber()+" must be filled, ");
					flag = true;
				}
			}
		}
