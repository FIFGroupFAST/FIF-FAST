...
public class AssetRetirementNonEBSApprovalComposer extends SelectorComposer<Window> {
	...
	@Wire
	private Window winAssetRetirementNonEBSApproval;
	...
	private void setTmpFilePath(){
		List<GlobalSettingDTO> dataImgPath = retirementNonEBSService.getGlobalsettingByCode(imageCode);
		if(dataImgPath!=null){
	...
	private void init(){
		...
		if(rd.getAvmTrxIdBast()!=null){
		    retirementDTO = retirementNonEBSService.getRetirementByAvmTrxIdBast(rd.getAvmTrxIdBast().toString());
		}else{
			retirementDTO = retirementNonEBSService.getRetirementByAvmTrxId(rd.getAvmTrxId().toString());
		}
	...
	public void submit() {
		...
		try {
		 retirementNonEBSService.save(retirementDTO, securityServiceImpl.getSecurityProfile().getPersonUUID(),listDetail,lstRetirementQuotation.getValue());
		} catch (FifException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (Exception e) {
	...
	public void back() {
		...
		winAssetRetirementNonEBSApproval.detach();
	}
