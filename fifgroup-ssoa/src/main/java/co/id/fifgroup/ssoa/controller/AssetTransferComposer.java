...
public class AssetTransferComposer extends SelectorComposer<Component> {
	...
	private static Logger log = LoggerFactory.getLogger(AssetTransferComposer.class);
	...
	private void init() {
		...
		Branch branch1 = (Branch) assetTransferService.getBranchById(securityServiceImpl.getSecurityProfile().getBranchId(),securityServiceImpl.getSecurityProfile().getWorkspaceCompanyId());
	...
	public void onDetail(ForwardEvent event){
		...
		Executions.createComponents("~./hcms/ssoa/asset_transfer_add.zul", getSelf().getParent(), param);
		getSelf().detach();
	}
    ...
    private void search() {
		try {
			...
			pgListOfValue.setTotalSize(assetTransferService.countByExample(searchCriteria()));
	...
	private void generateDataAndPaging() {
		...
		List listAssetTransferDto = assetTransferService.getAssetTransferDtoByExample(searchCriteria(),pgListOfValue.getPageSize(), pgListOfValue.getActivePage() * pgListOfValue.getPageSize());
	...
	private void generateDataAndPagingTop() {
		...
		List listAssetTransferDto = assetTransferService.getAssetTransferDtoByExample(searchCriteria(),pgListOfValueTop.getPageSize(), pgListOfValueTop.getActivePage() * pgListOfValueTop.getPageSize());
		ListModelList<AssetTransferDTO> model = new ListModelList<AssetTransferDTO>(listAssetTransferDto);
		lbxAssetTransfer.setModel(model);
		lbxAssetTransfer.renderAll();
	}
	...
	private void generateDataForDownload(){
		listAssetDtoForDownload = assetTransferService.getAssetTransferDtoByExample(searchCriteria());
	...
	public void popupSubmit(Event e) {
		Window win = (Window) Executions.createComponents("~./hcms/ssoa/asset_transfer_popup_status.zul", null, null);
		win.doModal();
	}
	...
	public void detail (Event e) {
		Window win = (Window) Executions.createComponents("~./hcms/ssoa/asset_transfer_add.zul", null, null);
		win.doModal();
	}
	...
	public void onPrintBast(ForwardEvent event) throws Exception{
		AssetTransferDTO assetTransferDTO = (AssetTransferDTO) event.getData();
		assetTransferService.printBAST(assetTransferDTO);
