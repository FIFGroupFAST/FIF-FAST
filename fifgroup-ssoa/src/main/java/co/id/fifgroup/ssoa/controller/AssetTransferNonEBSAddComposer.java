...
public class AssetTransferNonEBSAddComposer extends  SelectorComposer<Window> {
	...
	public void doAfterCompose(Window comp) throws Exception {
			...
			if(arg.get("action")!=null){
			    ...
			    if(action.equals("addNew")){
				    ...
				    Branch branch = (Branch)assetTransferNonEBSService.getBranchById(assetTransferDto.getBranchId(), securityServiceImpl.getSecurityProfile().getWorkspaceCompanyId());
				    KeyValue value = new KeyValue();
				    value.setKey(branch.getBranchId());
				    value.setValue(branch.getBranchName());
				    value.setDescription(branch.getBranchCode());
					...
					if(action.equals("detail")){
						...
						listDetailBast = assetTransferNonEBSService.getDetailBastByPrimaryKey(assetTransferBast.getTransferId());
						listModelBast.clear();
							...
							assetsList.addAll((List<AssetTransferDetailDTO>)event.getData());
								...
								for (int i = 0; i < ((List<AssetTransferDetailDTO>) event.getData()).size(); i++) {
									...
									for(int x=0;x<assetsList.size();x++){
										...
										System.out.println(asset.getAssetId()+"=="+assetTemp.getAssetId());
	...
	private void setTmpFilePath(){
		List<GlobalSettingDTO> dataImgPath = assetTransferNonEBSService.getGlobalsettingByCode(SSOAConstants.imagePathTmp);
		if(dataImgPath!=null){
			tmpFile = dataImgPath.get(0).getGlobalSetting().getValue().toString();
		}
		List<GlobalSettingDTO> dataMaxSizeUpload = assetTransferNonEBSService.getGlobalsettingByCode(SSOAConstants.maxSizeUploadFileTransfer);
	...
	class DelegateSearch1 implements SerializableSearchDelegateDoubleCriteria<Branch> {
	 ...
	 @Listen("onDetailBast= #winAssetTransferAdd")
		public void onDetail(ForwardEvent event){
			...
			Window win = (Window) Executions.createComponents("~./hcms/ssoa/asset_transfer_non_ebs_bast_popup_image.zul",
	...
	private void loadAssetTransfer() {
		...
		//String action = (String) arg.get("action");
		if(selected != null) {
			assetTransfer = assetTransferNonEBSService.getAssetTransferById(selected.getTransferId());
					...
					public void onEvent(Event event) throws Exception {
						...
						if (status == 16) {
							try {
								...
								assetTransferNonEBSService.submit(assetTransferDto,personUUID, lbxAssetAdd.getValue());
								...
								Executions.createComponents("~./hcms/ssoa/asset_transfer_non_ebs.zul", getSelf().getParent(),
										param);
								getSelf().detach();
		...
		Messagebox.show("Are you sure want to cancel?", "Information", Messagebox.YES | Messagebox.NO, Messagebox.INFORMATION, new SerializableEventListener<Event>() {
			...
			public void onEvent(Event event) throws Exception {	
				...
				if(status == 16) {
					Map<String, Object> param = new HashMap<String, Object>();
					param.put(GlobalVariable.FUNCTION_PERMISSION_KEY, functionPermission);
					Executions.createComponents("~./hcms/ssoa/asset_transfer_non_ebs.zul", getSelf().getParent(), param);
					getSelf().detach();
				} else {
					return;
				}
	...
	public void onshowInput() {
		...
		Window win = (Window) Executions.createComponents("~./hcms/ssoa/asset_transfer_non_ebs_popup.zul", getSelf().getParent(), param);
		win.doModal();
	}
					...
					Messagebox.QUESTION, new SerializableEventListener<Event>() {
						...
						public void onEvent(Event event) throws Exception {
							...
							if (status == 16) {
								...
								assetTransferNonEBSService.submitBast(assetTransferBastDTO);
								Map<String, Object> param = new HashMap<String, Object>();
								param.put(GlobalVariable.FUNCTION_PERMISSION_KEY, functionPermission);
